// Serviço de Armazenamento de Vídeos Locais via IndexedDB
// Suporta arquivos pesados (.mp4, .webm, .mov) sem estourar o limite de 5MB do LocalStorage

const DB_NAME = 'omc_media_db';
const DB_VERSION = 1;
...
class VideoStorageService {
  ...
  async saveVideoFile(id: string, file: File | Blob, name?: string): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const record: StoredVideo = {
        id,
        blob: file,
        name: name || (file instanceof File ? file.name : 'video.mp4'),
